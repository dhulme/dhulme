...
defineProps<{
  size?: 'small' | 'medium'
  member: DefaultTheme.TeamMember & { link: string }
}>()
</script>
    ...
    <div class="profile">
      ...
      <div class="data">
        ...
        <p v-if="member.title || member.org" class="affiliation">
          <div v-if="member.title" class="title" :style="{ marginBottom: '0.25rem' }">
            {{ member.title }}
          </div>
        ...
        <a :href="`pathname:///${member.link}/`">dhulme.uk/{{ member.link }}</a>
      </div>
    </div>
  </article>
</template>
